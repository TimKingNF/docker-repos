...
if [ -f "${SPARK_APPLICATION_JAR_LOCATION}" ]; then
    ...
    eval "/spark/bin/spark-submit --class ${SPARK_APPLICATION_MAIN_CLASS} --master ${SPARK_MASTER_URL} ${SPARK_SUBMIT_ARGS} ${SPARK_APPLICATION_JAR_LOCATION} ${SPARK_APPLICATION_ARGS}"
else
    if [ -f "${SPARK_APPLICATION_PYTHON_LOCATION}" ]; then
        ...
        echo "Passing arguments ${SPARK_APPLICATION_ARGS}"
        eval "PYSPARK_PYTHON=python3 /spark/bin/spark-submit --master ${SPARK_MASTER_URL} ${SPARK_SUBMIT_ARGS} ${SPARK_APPLICATION_PYTHON_LOCATION} ${SPARK_APPLICATION_ARGS}"
    else
